...
def buffer_from_bucket(bucket, header):
    ...
    number_of_channels = acquisitions[0].data.shape[0]
    number_of_samples = acquisitions[0].data.shape[1]
    buffer = np.zeros(
...
def combine_channels(image):
    return np.sqrt(np.sum(np.square(np.abs(image)), axis=3))


def split_image_array(image_data):
    return [image for image in image_data.reshape(*image_data.shape[:4], -1).transpose((4, 2, 1, 0, 3))]


def create_ismrmrd_image(image_data, reference_acquisition_header):
    return ismrmrd.image.Image.from_array(image_data, acquisition=reference_acquisition_header, transpose=False)
...
def reconstruct(buffers: gadgetron.types.ReconData, header):
    return flatten(
        [
            [image for image in reconstruct_buffer(buffer.data.data, buffer.data.headers.flat[0])]
