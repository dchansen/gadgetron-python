...
def read_vector(source, obj_type):
    ...
    if dtype == np.object or not dtype.isbuiltin or subtype == str:
        return np.array([read(source, subtype) for s in range(size)],dtype=np.object)
    else:
...
def read_array(source, obj_type):
    ...
    if dtype == np.object or not dtype.isbuiltin:
